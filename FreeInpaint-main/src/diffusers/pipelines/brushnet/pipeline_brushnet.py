...
class StableDiffusionBrushNetPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Below is for LLAVA
        # if val_llm_outputs is None:
        #     prompt_llava = f"USER: <image>\n A masked image with the black area representing the masked part. Caption describing the whole image:{prompt}. What should the masked part be to make the image complete and harmonious? ASSISTANT: *"
        #     llava_output = self.llava_model(prompt_llava, image).to(device)
        # else:
        #     llava_output = val_llm_outputs.to(device)
        # llava_output = llava_output.unsqueeze(1)  # [batchsize, 1, 1024]
        # llava_output = self.connector(llava_output, prompt_embeds).to(device)  # [batchsize, 1, 768]
        # prompt_embeds = torch.cat([prompt_embeds, llava_output], 1) # [batchsize, 78, 768]
        # batch_size, extra_tokens, embedding_dim = llava_output.shape
        # negative_prompt_embeds = torch.cat([negative_prompt_embeds, torch.zeros((batch_size, extra_tokens, embedding_dim), device=negative_prompt_embeds.device)], dim=1)

        # encode image and concat as negative prompt embeds
        # if isinstance(brushnet, BrushNetModel):
        #     dtype = next(self.image_encoder.parameters()).dtype
        #     if not isinstance(image, torch.Tensor):
        #         image_clip = self.feature_extractor(image, return_tensors="pt").pixel_values
        #     image_clip = image_clip.to(device=device, dtype=dtype)
        #     image_embeds_clip = self.image_encoder(image_clip).image_embeds
        #     print(f'Image Embeds Clip: {image_embeds_clip.shape}')
        #     print(f'Negative Prompt Embeds: {negative_prompt_embeds.shape}')
        #     negative_prompt_embeds = negative_prompt_embeds + image_embeds_clip
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # image.save(f"/export/CPFS/gongchao/test/238_{t}.png")
